...
export function StatesSection({ language, onNavigate }: StatesSectionProps) {
	...
	return (
		<section className="grid gap-3">
			...
			<div className="order-2 grid items-start gap-3 lg:order-1 xl:grid-cols-[minmax(0,1fr)_21rem]">
...
function GovernanceFields({ record, compact = false, variant = 'tiles' }: { record: StateGovernanceRecord; compact?: boolean; variant?: 'tiles' | 'plain' }) {
	const allRows = getStateGovernanceRows(record);
	const rows = compact ? allRows.filter((row) => row.field !== 'lead_party' && row.field !== 'chief_minister') : allRows;
	return (
		<dl className={cx('grid gap-2', compact ? 'mt-4 grid-cols-1' : variant === 'plain' ? 'grid-cols-[minmax(0,1.4fr)_minmax(0,1.4fr)_minmax(0,1fr)_minmax(0,1.6fr)_minmax(0,1.4fr)] sm:grid-cols-[minmax(0,1.4fr)_minmax(0,1.4fr)_minmax(0,1fr)_minmax(0,1.6fr)_minmax(0,1.4fr)]' : 'sm:grid-cols-2 xl:grid-cols-4')}>
			{rows.map((row) => {
				...
				return (
					<div className={cx('min-w-0', variant === 'tiles' ? 'rounded-md border border-[var(--bz-border)] bg-[var(--bz-surface-2)] px-2 py-1.5' : 'py-0.5')} key={row.field}>
						...
						<dd className={cx('mt-0.5 text-[11px] font-semibold text-[var(--bz-text-1)]', compact ? 'break-words' : 'truncate')} title={displayValue}>
							{displayValue}
						</dd>
					</div>
				);
