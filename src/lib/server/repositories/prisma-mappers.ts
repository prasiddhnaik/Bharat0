...
const VALID_BILL_ACTION_TYPES = new Set<BillAction['action_type']>([
	'sitting_scheduled', 'agenda_published', 'bill_introduced', 'bill_listed', 'bill_taken_up',
	'bill_referred_committee', 'committee_report_tabled', 'question_listed', 'question_answered',
	'debate_published', 'bill_passed_origin_house', 'bill_transmitted', 'bill_passed_second_house',
	'bill_assented', 'act_published', 'bill_withdrawn', 'bill_lapsed',
	'money_bill_window', 'president_assent'
]);

function toBillActionType(value: string): BillAction['action_type'] {
	if (VALID_BILL_ACTION_TYPES.has(value as BillAction['action_type'])) {
		return value as BillAction['action_type'];
	}
	throw new Error(`Unknown BillAction action_type: "${value}"`);
}
...
export function toDomainBillAction(row: PrismaBillAction): BillAction {
	return {
		...
		action_type: toBillActionType(row.action_type),
