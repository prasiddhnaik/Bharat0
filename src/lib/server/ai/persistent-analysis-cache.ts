...
import type { AiAnalysisProvider, GroqBillAnalysis, GroqBillAnalysisPayload } from './groq-bill-analysis';
...
export async function readPersistedBillAnalysis(prisma: PrismaAiClient, key: AnalysisCacheKey): Promise<GroqBillAnalysisPayload | null> {
	...
	if (!row || !isGroqBillAnalysis(row.analysis)) {
		return null;
	}
...
export async function persistBillAnalysis(prisma: PrismaAiClient, key: AnalysisCacheKey, payload: GroqBillAnalysisPayload) {
...
function isGroqBillAnalysis(value: Prisma.JsonValue): value is GroqBillAnalysis {
